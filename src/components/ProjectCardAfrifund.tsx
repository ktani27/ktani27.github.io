...
export function ProjectCardAfrifund() {
    const { t } = useLanguage();
    const data = t.projects.afrifund;
    ...
    return (
        <div className="flex flex-col h-full bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl group">
            ...
            <div className="h-48 w-full bg-gradient-to-br from-blue-600 to-indigo-800 relative overflow-hidden flex items-center justify-center">
                ...
                <span className="text-white text-2xl font-bold tracking-widest uppercase z-10 glass px-6 py-2 rounded-lg">{data.title}</span>
            </div>
                ...
                <div className="space-y-4">
                    ...
                    <ul className="space-y-1 text-sm text-muted-foreground">
                        {data.points.map((point: string, idx: number) => (
                            <li key={idx} className="flex items-start">
                                <CheckCircle2 className="w-4 h-4 mr-2 text-primary mt-0.5 shrink-0" /> {point}
                            </li>
                        ))}
                    </ul>
                    ...
                    <div className="pt-4 border-t border-border/50 flex gap-3 mt-auto">
                        <Link href="#" className="flex-1 inline-flex justify-center items-center py-2 px-4 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors">
                            <Link2 className="w-4 h-4 mr-2" /> {data.btnLive}
                        </Link>
